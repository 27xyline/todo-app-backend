...
@router.get("", response_model=list[TaskRead], status_code=status.HTTP_200_OK, tags=["Задачи"])
def get_tasks(service: TaskService = Depends(get_task_service)) -> list[TaskRead]:
    return service.list_tasks()


@router.post("", response_model=TaskRead, status_code=status.HTTP_201_CREATED, tags=["Задачи"])
def create_task(payload: TaskCreate, service: TaskService = Depends(get_task_service)) -> TaskRead:
    return service.create_task(payload)


@router.patch("/{task_id}", response_model=TaskRead, status_code=status.HTTP_200_OK, tags=["Задачи"])
def update_task(task_id: UUID, payload: TaskUpdate, service: TaskService = Depends(get_task_service)) -> TaskRead:
    try:
